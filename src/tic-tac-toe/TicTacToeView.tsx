...
const cellStyle = (value: string | null, isWinning: boolean): React.CSSProperties => ({
  width: 80,
  height: 80,
  fontSize: 32,
  fontWeight: 700,
...
export const TicTacToeView = ({
  ...
  const isMyTurn = myPlayer && state.currentPlayer === myPlayer.mark && !state.winner;
    ...
    const joinTimeout = setTimeout(() => {
      sendAction({
        ...
        meta: {
          playerId,
          timestamp: Date.now(),
          actionId: `join:${playerId}:${Date.now()}`
        }
      });
    }, 500);
    ...
    (row: number, col: number) => {
      ...
      sendAction({
        ...
        meta: {
          playerId,
          timestamp: Date.now(),
          actionId: `place:${playerId}:${row}:${col}:${Date.now()}`
        }
      });
    },
  ...
  const resetGame = useCallback(() => {
    sendAction({
      ...
      meta: {
        playerId,
        timestamp: Date.now(),
        actionId: `reset:${playerId}:${Date.now()}`
      }
    });
  }, [playerId, sendAction]);
  ...
  }, [state.winner, state.players, state.currentPlayer, myPlayer, isMyTurn, displayName]);
