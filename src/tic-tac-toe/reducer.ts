  ...
  switch (action.type) {
    ...
    case 'join.game': {
      ...
      const takenMarks = new Set(
        Object.values(state.players).map((p) => p.mark)
      )
      const mark =
        action.payload.preferredMark &&
        !takenMarks.has(action.payload.preferredMark)
          ? action.payload.preferredMark
          : takenMarks.has('X')
            ? 'O'
            : 'X'
    ...
    default:
      return {
        ...state,
        appliedActionIds: appendId(
          state,
          (action as TicTacToeAction).meta.actionId
        ),
      }
  }
}
...
export const replayActions = (
  ...
  actions.reduce<TicTacToeState>((s, a) => ticTacToeReducer(s, a), initialState)
