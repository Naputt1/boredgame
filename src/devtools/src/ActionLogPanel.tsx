...
export const ActionLogPanel = <TAction,>({
  ...
  const [entries, setEntries] = useState<ActionLogEntry<TAction>[]>(
    actionLog.getAll()
  )
  ...
  if (entries.length === 0) {
    return (
      <div className="boredgame-devtools-empty">No actions recorded yet</div>
    )
  }
  ...
  return (
    <div className="boredgame-devtools-body">
      {entries.map((entry) => {
        const actionType = (entry.action as { type?: string }).type ?? 'unknown'
        ...
        return (
          <div key={entry.index}>
            ...
            >
              <span className="boredgame-devtools-action-index">
                #{entry.index}
              </span>
              <span className="boredgame-devtools-action-type">
                {actionType}
              </span>
              <span className="boredgame-devtools-action-time">
                {timeStr(entry.timestamp)}
              </span>
            </div>
            <div
              className={`boredgame-devtools-action-preview ${isExpanded ? 'open' : ''}`}
            >
