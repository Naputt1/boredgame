...
export class Room {
  ...
  private migrateHost(): void {
    ...
    for (const [pid, slot] of this.slots) {
      if (!slot.isSpectator) {
        const existing = newHost ? this.slots.get(newHost) : undefined
  ...
  setSpectate(playerId: string, spectating: boolean): string | null {
    ...
    if (
      spectating &&
      this.spectatorCount >= this.maxSpectators &&
      !slot.isSpectator
    ) {
      return 'SPECTATOR_FULL'
    }
  ...
  handleAction(rawAction: unknown, senderPlayerId: string): void {
    ...
    try {
      ...
      this.sendToPlayer(senderPlayerId, {
        ...
        payload: {
          code: 'INVALID_ACTION',
          message: 'Action failed schema validation',
        },
      })
      return
    }
    ...
    if (this.definition.validateAction) {
      const result = this.definition.validateAction(
        action,
        this.state,
        senderPlayerId
      )
  ...
  private pruneActionLog(): void {
    const snapshotState = this.actionLog.reduce<unknown>(
      (s, a) => this.definition.reducer(s, a),
