...
import type { GameDefinition, RoomLifecycleState } from "@boredgame/core";
import {
  type ClientMessage,
  type Envelope,
  WSPROTO_VERSION,
...
wss.on("connection", (socket: WebSocket, req) => {
  ...
  const connectionDiscordInstanceId = url.searchParams.get("discordInstanceId") || undefined;
    ...
    switch (msg.type) {
      case "create-room": {
        if (clientVer !== WSPROTO_VERSION) {
          sendError(socket, "BAD_VERSION", `Expected version ${WSPROTO_VERSION}, got ${clientVer}`);
          return;
        }
        ...
        sendEnvelope(socket, WSPROTO_VERSION, 0, {
          type: "join-room-ack",
          payload: { chosenVersion: WSPROTO_VERSION, playerId: msg.payload.playerId }
        });
      ...
      case "join-room": {
        ...
        if (!room) {
          room = new Room(roomId, gameDef, msg.payload.playerId, undefined, () => {
            rooms.delete(roomId);
            if (room?.discordInstanceId) {
              roomsByDiscordInstance.delete(room.discordInstanceId);
            }
      ...
      case "spectate": {
        ...
        const err = room.setSpectate(msg.payload.playerId, msg.payload.spectating);
        if (err) {
  ...
  for (const [roomId, room] of rooms) {
    ...
    for (const slot of room.getSlots()) {
      ...
      if (last && now - last > HEARTBEAT_TIMEOUT_MS) {
        console.log(`Stale connection detected for ${slot.playerId} in room ${roomId}`);
        socket.terminate();
