        ...
        {games.map((game) => (
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <strong style={{ fontSize: 18 }}>{game.id === "boredgame-demo" ? "Demo Board" : game.id === "boredgame-tictactoe" ? "Tic-Tac-Toe" : game.id}</strong>
            ...
            {game.tags && game.tags.length > 0 && (
              <div style={{ display: "flex", gap: 4, flexWrap: "wrap", marginTop: 4 }}>
                {game.tags.map((tag) => (
                  <span key={tag} style={{ ...badgeStyle, background: "#1f2937", fontSize: 10 }}>
                    {tag}
                  </span>
                ))}
              </div>
