...
export const GameProvider = <TState, TAction>({
  ...
  const engine = useMemo<GameEngine<TState, TAction>>(
    ...
    [syncMode, transport, definition, middleware, initialGameState]
  )
  ...
  const isSpectator = roomPlayers.some(
    (p) => p.playerId === playerId && p.isSpectator
  )
  ...
  const setSpectateCallback = useCallback(
    (spectating: boolean) => {
  ...
  const value = useMemo<GameContextValue>(
    () => ({
      ...
      sendAction: (action: unknown) => {
        ;(engine.sendAction as (action: unknown) => void)(action)
      },
      connected,
      connectionStatus,
      playerId,
...
export const useGameEngine = <
  TState = unknown,
  TAction = unknown,
>(): GameEngine<TState, TAction> | null => {
